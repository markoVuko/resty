...
{
    public class EditItemValidator : AbstractValidator<ItemDto>
    {
        public EditItemValidator(RadContext con)
        {
            RuleFor(x => x.Name)
                .NotEmpty()
                .WithMessage("Item name can not be empty!").DependentRules(() =>
                {
                    ...
                    {
                        return !con.Items.Any(r => r.Id!=a.Id&&r.Name.ToLower().Contains(b.ToLower()));
